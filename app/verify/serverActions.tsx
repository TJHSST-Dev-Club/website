...
export async function getAuthCodeUrl(): Promise<string> {
  ...
  const login_url: string = oauth.authorizeURL({
    scope: "read",
...
export async function getDiscordCode(
  ...
  try {
    ...
    console.log(error);
    return ["Failed to get authorization code", false];
  }
  ...
  if (response.status != 200) {
    return ["Failed to get Ion data with code " + response.status, false];
  }
  ...
  return [discordVerificationCode, true];
}
